...
% Strong support for our first hypothesis. The graphs with high number of links (Geant, Iij) converged quickly, while the graphs with low number of links converged slowly or not at all during the experiment.
% Entropy: What to say here?
% Real world implications: Gossip algorithm should be only used in densely connected networks or on tasks where slow information dissemination is acceptable. It should not be used on scarcely connected networks on time sensitive tasks.
% The implementation in a real computer network held some surprises.
% This experiment is limited by the random nature of the algorithm. As such the experiment should be repeated many times to solidify the results.
